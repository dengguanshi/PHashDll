...
String  init2(Mat src, Pic PicArray[]);
...
(JNIEnv* env, jobject obj, jobject jobj1, jobject jobjList) {
	...
	for (int i = 0; i < len; i++) {
		...
		PicArray[i] = MyJobject2Mat2(env, obj_user);
		cout << "=PicArray[i]=="<< PicArray[i] .id<< endl;
	}

	String outputpath = init2(MyJobject2Mat2(env,jobj1).picMat, PicArray);
	// 返回一个字符串
	char const* tmpstr = outputpath.c_str();
	jstring rtstr = env->NewStringUTF(tmpstr);
	return rtstr;
...
String  init2(Mat src, Pic PicArray[]) {
	...
	Mat srchash, dsthash;
	fingerprint(src, &srchash);
	int min = 100; string mini;
	for (int i = 1; i <= 2; i++)
	{
		
		Mat dst = PicArray[i-1].picMat;
		if (dst.empty())
		{
			cout << "the image is not exist" << endl;
			return "";
